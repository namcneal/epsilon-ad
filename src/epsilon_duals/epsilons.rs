...
pub struct NonEmptyEpsilonProduct(pub (crate) [EpsilonFieldType; 2]);
pub (crate) type Epsilon = NonEmptyEpsilonProduct;

impl Deref for NonEmptyEpsilonProduct{
    type Target = [EpsilonFieldType; 2];
...
impl NonEmptyEpsilonProduct {
    pub fn singleton(order:u8, direction:u8) -> NonEmptyEpsilonProduct{
        NonEmptyEpsilonProduct([order, direction])
    }
}   
